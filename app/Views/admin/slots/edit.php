...
<div class="card">
    ...
    <div class="card-body">
        <form action="<?= base_url('/admin/delivery-slots/edit/' . $slot['id']) ?>" method="post">
            <div class="mb-3">
                <label for="slot_time" class="form-label">Slot Time</label>
                <input type="text" class="form-control" id="slot_time" name="slot_time" value="<?= old('slot_time', $slot['slot_time']) ?>" required>
                <div class="form-text">Example: 8:00 AM - 9:00 AM</div>
            </div>
            
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" <?= old('is_active', $slot['is_active']) ? 'checked' : '' ?>>
